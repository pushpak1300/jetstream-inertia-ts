...
import { PageProps, Permissions, Role, Team, User } from '@/types';
...
const addTeamMemberForm = useForm<AddTeamMemberForm>({
    email: '',
    role: null,
});
...
const currentlyManagingRole = ref<boolean>(false);
const managingRoleFor = ref<User | null>(null);
const confirmingLeavingTeam = ref<boolean>(false);
...
const cancelTeamInvitation = (invitation: any): void => {
    router.delete(route('team-invitations.destroy', invitation), {
        preserveScroll: true,
    });
};

const manageRole = (teamMember: User): void => {
    managingRoleFor.value = teamMember;
    updateRoleForm.role = teamMember?.membership?.role ?? null;
...
const confirmTeamMemberRemoval = (teamMember: User): void => {
    teamMemberBeingRemoved.value = teamMember;
};

const removeTeamMember = (): void => {
    if (!teamMemberBeingRemoved.value) return;
...
const displayableRole = (role: string): string | undefined => {
